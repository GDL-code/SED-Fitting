...
def ln_likelihood(vLv_norm,vLv_e_norm,up_limit,vLv_tem):
    global reduced_number
    if len(up_limit)==0:
        model         =vLv_tem
        sigma2        =vLv_e_norm** 2
        reduced_number=len(vLv_norm)
...
#    S_grey        =np.random.uniform(ratio_g,100*ratio_g,walkers_num)
    ...
    pos               =pos_2.T
    return pos
def emcmc(tem_control,Av_control,source_dict,global_dict,index,chi_square=0,theta=None):
    ...
    initial_step  =int(global_dict['initial_step'])
    redshift      =source_dict['redshift']
    if chi_square==0:
        ...
        pos,prob,state=sampler.run_mcmc(pos, initial_step ,progress=False)
        sampler.reset()
        if index==0:
        ...
        chi_square=-2.0*ln_probability(theta,tem_control,Av_control,tem_vLv,vLv_norm,vLv_e_norm,wave_rest,up_limit)
        re_chi    =chi_square/(reduced_number-par_number-1)
        return chi_square,re_chi
